...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        cv::Mat image,gray,fgbgmask,threshold_image,accum_image,color_image1,frame,result_overlay,final_img;
        if (!cap.read(image)) {
        ...
        do {
	        ...
	        applyColorMap(accum_image, color_image1, COLORMAP_HOT);
	        addWeighted(image, 0.7, color_image1, 0.3, 0.0, result_overlay);

            //heatmap integration ends here

            cv::Mat fpsPane(35, 155, CV_8UC3);
            fpsPane.setTo(cv::Scalar(153, 119, 76));
            cv::Mat srcRegion = result_overlay(cv::Rect(8, 8, fpsPane.cols, fpsPane.rows));
            cv::addWeighted(srcRegion, 0.4, fpsPane, 0.6, 0, srcRegion);
	    
	        std::stringstream fpsSs;
            fpsSs << "FPS: " << int(1000.0f / inferenceTime * 100) / 100.0f;
            cv::putText(result_overlay, fpsSs.str(), cv::Point(16, 32),
                        cv::FONT_HERSHEY_COMPLEX, 0.8, cv::Scalar(0, 0, 255));
            cv::imshow("ICV Human Pose Estimation", result_overlay);
